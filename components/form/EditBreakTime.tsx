...
import { BreakTime } from "@/lib/types/type";
...
const EditBreakTime = forwardRef<HTMLFormElement, Props>((props, ref) => {
  const { theme } = useTheme();
  const formattime = "HH:mm";
  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      monitor: props.data.montrid,
    },
  });
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-4xl">
        <DialogHeader>
          <DialogTitle>Edit BreakTime</DialogTitle>
          <DialogDescription>
            Make changes to your BreakTime here. Click Submit when youre done.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
      <form
